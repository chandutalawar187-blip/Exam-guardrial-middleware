...
export default function CreateStudentPanel() {
  const [form, setForm] = useState({ user_id: '', name: '', password: '', exam_name: '' })
  ...
  const createStudent = async () => {
    try {
      ...
      setForm({ user_id: '', name: '', password: '', exam_name: form.exam_name }) // Keep exam name for batch entry
      setErr('')
              ...
              <div key={s.id} className="bg-[#eff6ff] border border-[#7BBDE8] p-4 rounded-xl flex justify-between items-center transition-all">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-[#49769F] font-black uppercase tracking-widest">{s.user_id} • {s.exam_name}</p>
                  </div>
                </div>
