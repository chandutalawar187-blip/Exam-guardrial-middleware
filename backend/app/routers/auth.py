...
async def student_login(body: dict):
    try:
        ...
        # If subject_code is provided, look up exam by subject_code
        ...
        schedule_lookup = db.table('exam_questions').select('start_time, end_time').eq('exam_name', target_exam).limit(1).execute()
        start_time = None
        end_time = None
        if schedule_lookup.data:
            start_time = schedule_lookup.data[0].get('start_time')
            end_time = schedule_lookup.data[0].get('end_time')
        ...
        print(f'[auth] student-login error: {e}')
